...
Although AI isn't part of building data mesh solutions, we can't finish without connecting the thing that has pushed data mesh out of the spotlight, AI via LLMs like ChatGTP. The DSL approach presented in this article flows smoothly into the ChatGTP excitment. As discussed in detail in the following, DSLs are an ideal way to use LLMs and ChatGTP to generate code that a business SME can actually understand. 
